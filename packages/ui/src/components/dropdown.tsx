...
export function DropdownItem({
  ...
  if ('to' in props) {
    return (
      <Headless.MenuItem
        as={Link}
        {...(props as Omit<Headless.MenuItemProps<typeof Link>, 'as' | 'className'>)}
        className={classes}
      />
    )
  }
  const buttonItemProps = props as Omit<Headless.MenuItemProps<'button'>, 'as' | 'className'>
  const { type: itemType, ...restOfButtonItemProps } = buttonItemProps
  return (
    <Headless.MenuItem
      as='button'
      type={itemType === 'submit' || itemType === 'reset' ? itemType : 'button'}
      {...restOfButtonItemProps}
      className={classes}
    />
  )
}
